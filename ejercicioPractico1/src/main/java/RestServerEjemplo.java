...
public class RestServerEjemplo {
    ...
    public static void main(String[] args) {
        ...
        post("/incidente", (request,response) -> {
            ...
            if(usuarioService.validateUsuario(incidente.getReportador())) {
                incidenteService.addIncidente(incidente);
                requestResponse = new Gson().toJson(new StandardResponse(StatusResponse.SUCCESS, "Incidente añadido"));
            }else{
                requestResponse = new Gson().toJson(new StandardResponse(StatusResponse.ERROR, "Error al añadir incidente."));
            }
            return requestResponse;
        });
        ...
        get("/incidente", (request, response) -> {
            response.type("application/json");
            return new Gson().toJson(new StandardResponse(StatusResponse.SUCCESS, new Gson().toJsonTree(incidenteService.getIncidente())));
        });

        //Buscar incidentes de un usuario con estado asignado.
        get("/incidente/:id", ((request, response) -> {
            String requestResponse = "";
            response.type("application/json");
            //valido si el usuario existe para evitar buscar entre todos los incidentes innecesariamente.
            if(usuarioService.validateUserIdExists(Integer.parseInt(request.params("id"))) &&
                    //valido si el usuario posee incidente asociados como responsable o reportador.
                    (  incidenteService.getUsuarioResponsable(usuarioService.getUsuario(Integer.parseInt(request.params("id")))) ||
                            incidenteService.getUsuarioReportador(usuarioService.getUsuario(Integer.parseInt(request.params("id"))))) ){
                requestResponse = new Gson().toJson( new StandardResponse (StatusResponse.SUCCESS,
                        new Gson().toJson( incidenteService.getIncidentesAsignadosUsuario( usuarioService.getUsuario( Integer.parseInt( request.params( "id" )))))));
            }
           return requestResponse;
        }));

        //Mostrar los incidentes con estado RESUELTO
        get("/incidente/incidentecerrado", ((request, response) -> {
            String requestResponse = "";
            response.type("application/json");
            requestResponse = new Gson().toJson( new StandardResponse (StatusResponse.SUCCESS,
        ...
        //Mostrar incidentes de determinado proyecto
        get("/incidente/:proyecto", ((request, response) -> {
            String requestResponse = "";
            response.type("application/json");
            requestResponse = new Gson().toJson( new StandardResponse(StatusResponse.SUCCESS,
                    new Gson().toJsonTree( incidenteService.getProyectoIncidentes( proyectoService.getProyecto( Integer.parseInt(request.params( ":proyecto" )))))));
            return requestResponse;
        }));
